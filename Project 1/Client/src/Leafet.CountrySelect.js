...
L.CountrySelect.countries = countries;
  ...
  onAdd: function (map) {
    ...
    for (i in countryKeys) {
      if (this.options.exclude.indexOf(countryKeys[i]) == -1) {
        let icon =
          country_icons[countryKeys[i]] == undefined
            ? ""
            : country_icons[countryKeys[i]];
        let src = `data-content="<img class='country-icon' src='https://purecatamphetamine.github.io/country-flag-icons/3x2/${icon}.svg'>${countryKeys[i]}"`;
        content += `<option ${src}>` + countryKeys[i] + `</option>`;
      }
    }

    this.select.innerHTML = content;
    $("#country-select-box").html(content).selectpicker("refresh");
